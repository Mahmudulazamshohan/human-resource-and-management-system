...
class DashboardController extends Controller
{
    ...
    public  function dashboard(){
        ...
        $data['jan'] = floor(StaffSchedule::where('month','like','%Jan '.date('Y').'%')->count());
        $data['feb'] = floor(StaffSchedule::where('month','like','%Feb '.date('Y').'%')->count()); 
        $data['mar'] = floor(StaffSchedule::where('month','like','%Mar '.date('Y').'%')->count());
        $data['apr'] = floor(StaffSchedule::where('month','like','%Apr '.date('Y').'%')->count());
        $data['may'] = floor(StaffSchedule::where('month','like','%May '.date('Y').'%')->count());
        $data['jun'] = floor(StaffSchedule::where('month','like','%Jun '.date('Y').'%')->count());
        $data['jul'] = floor(StaffSchedule::where('month','like','%Jul '.date('Y').'%')->count());
    ...
    public function editDepartments($id){
      $data['sts']=$this->setting;
       ...
       $data['departments'] = Department::find($id); 
        return view('admin.edit-departments',$data);
    }
    ...
    public function addAttendances(){
      $data['sts']=$this->setting;
    	...
    	return view('admin.add-attendances',$data);
    }
   ...
   public function addAbsences(){
     ...
     return view('admin.add-absences',$data);
   }
   public function storeAbsences(Request $r){
     ...
     $month  = Carbon::parse($r->date)->format('M Y');
   ...
   public function editAbsences($id){
    $data['sts']=$this->setting;
     ...
     return view('admin.edit-absences',$data);
   }
   ...
   public function rewards(){
    $data['sts']=$this->setting;
   	...
   	$data['employee'] =Employee::all();
   	return view('admin.rewards',$data);
   }
 ...
 public function staffScheduling(){
   ...
   return view('admin.staff-scheduling',$data);
 }
...
public function performanceBonus(){
  ...
  return view('admin.performance-bonus',$data);
}
...
public function editPerformanceBonus($id){
  ...
  $data['sts']=$this->setting;
	...
	return view('admin.edit-performance-bonus',$data);
...
public function salaryType(){
  ...
  $data['employee'] =  Employee::all();
  return view('admin.salary-type',$data);
}
public function storeSalaryType(Request $r){
  ...
  if(count(SalaryType::where('user_id',$r->user_id)->first())){
...
public function editSalaryType($id){
  ...
  return view('admin.edit-salary-type',$data);
}
...
public function viewSalaryStructure($id){
  ...
  $data['current_salary']  = DB::select('SELECT SUM(staff_schedules.hours_worked_int) as hours_worked,sum(staff_schedules.overtime_int) as overtime from staff_schedules WHERE staff_schedules.user_id = '.$id.' AND staff_schedules.month = (SELECT staff_schedules.month FROM staff_schedules WHERE staff_schedules.user_id = '.$id.' ORDER BY staff_schedules.month DESC LIMIT 1)')[0];
  ...
  $data['income_tax'] = IncomeTax::where('user_id',$id)->first();
  $data['leave_pay'] = LeavePay::where('user_id',$id)->first();
  $data['overtimes'] = Overtime::where('user_id',$id)->first();
  $data['salary_type'] =SalaryType::where('user_id',$id)->first();
  $data['absence'] = DB::select('SELECT count(user_id) as ab FROM absences where user_id = 27 and month = (SELECT month from absences WHERE user_id =27 order by month DESC LIMIT 1)')[0]->ab;
 
   return view('admin.view-salary',$data);
}
...
public function leavesEdit($id){
 ...
 $data['leave_type'] =LeaveType::all();
 $data['sts']=$this->setting;
 return view('admin.edit-leaves',$data);
}
...
public function editAnnouncement($id){
  ...
  $data['id'] = $id;
  return view('admin.edit-announcement',$data);
}
...
public function  travelExpenses(){
  ...
  return view('admin.travel-expenses',$data);
}
...
public function editTravelExpenses($id){
  ...
  return view('admin.edit-travel-expenses',$data);
}
...
public function meetingRecord(){
 ...
 $data['em'] = Employee::all();
 $data['sts'] = $this->setting;
 return view('admin.meeting-record',$data);
}
...
public function violationRecord(){
  ...
  return view('admin.violation',$data);
...
public function storeViolationRecord(Request $r){
 ...
 public function healthInsurances(){
  ...
  $data['em'] =Employee::all();
  $data['hl']  =Health::all();
  return view('admin.health-insurances',$data);
 }
...
public function incomeTax(){
  ...
  $data['employee'] =Employee::all();
  $data['hl']  =Health::all();
  $data['income'] = IncomeTax::all();
  return view('admin.income-tax',$data);
}
public function editIncomeTax($id){
  $data['income'] = IncomeTax::find($id);
  $data['title'] = 'Income Tax Edit';
  $data['sts'] =$this->setting;
  $data['employee']  = Employee::all();
  return view('admin.edit-income-tax',$data);
}
public function deleteIncomeTax($id){
    IncomeTax::find($id)->delete();
    session()->flash('type','success');
    session()->flash('message','Your Successfully Tax Percentage Delete ,Please Edit them or delete');
    return redirect()->back();
}
public function updateIncomeTax(Request $r){
   $this->validate($r,[
   'percentage'=>'required',
   'current_salary'=>'required'
  ]);
  IncomeTax::where('user_id',$r->employee_id)->update([
      'percentage'=>$r->percentage,
      'current_salary'=>$r->current_salary
   ]);
    session()->flash('type','success');
    session()->flash('message','Your Successfully Tax Percentage Updated');
    return redirect()->back();
}
public function storeIncomeTax(Request $r){
  $this->validate($r,[
   'user_id'=>'required',
   'percentage'=>'required',
   'current_salary'=>'required'
  ]);
   if(IncomeTax::where('user_id',$r->user_id)->first()!= null){
    session()->flash('type','danger');
    session()->flash('message','Your Already added Tax Percentage');
    return redirect()->back();
  }else{
    ...
    session()->flash('type','success');
    session()->flash('message','Your Successfully Tax Percentage Added');
    return redirect()->back();
  }
...
public function updateProfile(Request $r){
  ...
  session()->flash('type','success');
  session()->flash('message','Your Successfully Name And Password Updated ');
  return redirect()->back();
}
public function leavePay(){
  $data['title']= 'Leave Pay';
  $data['sts'] =$this->setting;
  $data['employee'] =  Employee::all();
  $data['leave_pay'] = LeavePay::all();

  return view('admin.leave-pay',$data);
}
public function storeLeavePay(Request $r){
 $this->validate($r,[
  'user_id'=>'required',
  'hourly_salary'
 ]);
 if(count(LeavePay::where("user_id",$r->user_id)->first())){
   session()->flash('type','danger');
   session()->flash('message','Leave Pay Deduction Already added, You can edit them');
 }else{
   LeavePay::create(Input::except("_token"));
   session()->flash('type','success');
   session()->flash('message','Leave Pay Deduction added Successfully');
 }
 return redirect()->back();

 
}
public function editLeavePay($id){
  $data['title']= 'Edit Leave Pay';
  $data['sts'] =$this->setting;
  $data['employee'] =  Employee::all();
  $data['lp'] = LeavePay::find($id);
  return view('admin.edit-leave-pay',$data);
}
public function updateLeavePay(Request $r){
 $this->validate($r,[
 'user_id'=>'required',
 'hourly_salary'=>'required'
 ]);
 LeavePay::where('user_id',$r->user_id)->update(Input::except('_token','user_id'));
 session()->flash('type','success');
 session()->flash('message','Leave Pay Updated Successfully');
 return redirect()->back();
}
public function deleteLeavePay($id){

}
public function overtimePay(){
  $data['title']= 'Overtime  Pay';
  $data['sts'] =$this->setting;
  $data['employee'] =  Employee::all();
  $data['overtime'] = Overtime::all();
  return view('admin.overtime-pay',$data);
}
public function storeOvertimePay(Request $r){
  $this->validate($r,[
   'user_id'=>'required',
   'hourly_salary'=>'required',
  ]);
  if(count(Overtime::where('user_id',$r->user_id)->first())){
     session()->flash('type','danger');
     session()->flash('message','Salary Type Already added, You can edit them');
  }else{
     Overtime::create(Input::except('_token'));
     session()->flash('type','success');
...
public function overtimeEdit($id){
  $data['title']= 'Overtime  Edit';
  $data['sts'] = $this->setting;
  $data['employee'] =  Employee::all();
  $data['overtime'] = Overtime::find($id);
  return view('admin.overtime-edit',$data);
}
public function updateOvertimePay(Request $r){
  $this->validate($r,[
  'hourly_salary'=>'required'
  ]);
  Overtime::where('user_id',$r->user_id)->update(Input::except("_token"));
  session()->flash('type','success');
  session()->flash('message','Overtime updated Successful');
